...
### Por que `expo run:android` e não `expo start`?

O SyncFlow usa bibliotecas **nativas** — em especial o WatermelonDB, que é o banco de dados local do app. Essas bibliotecas precisam ser compiladas junto com o código nativo do Android; o **Expo Go** (app padrão de desenvolvimento) não consegue carregá-las.

Por isso, o comando correto é sempre:
...
Esse comando:
1. Compila o app com todas as dependências nativas
2. Instala o **dev build** no emulador ou dispositivo conectado
3. Inicia o servidor de desenvolvimento apontado para esse build

> ⚠️ **Pré-requisitos:** Android Studio instalado + emulador configurado (ou dispositivo físico com depuração USB habilitada).

Depois da primeira compilação, nas próximas vezes o app já estará instalado e basta rodar o comando novamente — ou iniciar o servidor com `npx expo start --dev-client` se o build já estiver no dispositivo.
